...
export const QuizLevel3 = ({
  ...
  const washingDoneRef = useRef();
  const stylingDoneRef = useRef();
  ...
  const applyFormData = (e) => {
    ...
    sendWashingDone(washingDoneRef.current.checked);
    sendStylingDone(stylingDoneRef.current.checked);
  };
  ...
  return (
    <form className="document-input" onSubmit={(e) => applyFormData(e)}>
      ...
      <div id="activities-input">
        ...
        <input type="checkbox" id="washing" ref={washingDoneRef} />
        <label htmlFor="washing">Washed a car</label>
        <br />

        <input type="checkbox" id="styling" ref={stylingDoneRef} />
